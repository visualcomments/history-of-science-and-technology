# Индекс и эмбеддинги на Google Диске (рекомендуемая схема доступа)

Индекс корпуса (Annoy + эмбеддинги + чанки) распространяется как **файлы
на Google Диске**: репозиторий самодостаточен, сервер не требуется.
Агент (или пользователь) скачивает архив по ссылке, инструмент проверяет
контрольные суммы и разворачивает индекс в локальный корпус
(`FALT_CORPUS_ROOT/index/`), после чего работают `make search`,
`make verify` и локальный RAG-API.

## Что загружается на Диск

Один архив (рекомендуется) с именем `course-index-2026-09-01.zip`:

| Файл в архиве | Размер | SHA-256 |
|---|---|---|
| `annoy.index` | 43,2 МБ | `4B2BFF62…C26355` |
| `chunks.jsonl` | 45,7 МБ | `9090DB3D…B61CD` |
| `embeddings.npy` | 37,7 МБ | `839E34CF…2C6C3E` |
| `config.json` | 3,6 КБ | `60AECE04…4B6C` |

Архив: **86,3 МБ** (20 506 029 байт), `SHA-256 = AB070598F3774A625831E51DC6EBCB44538FE933D13A064CF46C58A1CD4E509C`.
(Полные контрольные суммы — в `index-manifest.example.json`.)

Актуальные пути файлов-источников на момент подготовки:
`<корпус>/index/annoy.index`, `.../chunks.jsonl`, `.../config.json`,
`.../embeddings.npy` (архив — `course-index-2026-09-01.zip`, 86,3 МБ). Это
исправленные и проверенные версии (64/64 цитат).

## Как выложить и подключить

1. **Загрузите архив** (или 4 файла по отдельности) на Google Диск;
   в настройках общего доступа выберите **«Все, у кого есть ссылка» →
   «Читатель»**.
2. **Скопируйте share-ссылку** (вид `https://drive.google.com/file/d/FILE_ID/view?usp=sharing`)
   или прямой download-link (`https://drive.google.com/uc?export=download&id=FILE_ID`).
3. **Подключите одним из способов:**
   - разово: `make index-fetch URL="<ссылка>"` (или `FALT_INDEX_URL=<ссылка>`);
   - постоянно: скопируйте `index-manifest.example.json` в `index-manifest.json`
     и впишите ссылку в поле `archive.url` — тогда работает просто `make index-fetch`.
4. Инструмент проверит SHA-256, распакует и **атомарно** заменит
   `FALT_CORPUS_ROOT/index/` (старая папка → `index_old/`).

После развёртывания: `make search QUERY="..."`, `make verify`,
`make serve` — как с любым локальным корпусом.

## Для владельца репозитория (обновление манифеста)

При новой версии индекса: пересобрать архив, пересчитать SHA-256
(`Get-FileHash` / `sha256sum`), обновить `index-manifest.example.json`,
загрузить архив на Диск, проставить новую ссылку в
`index-manifest.example.json` → коммит. Инструкция для пользователя в
“Обновление индекса” (ниже).

## Обновление индекса

1. Скачать новый архив по той же ссылке (или обновить ссылку в манифесте).
2. `make index-fetch` — сумма сверяется с манифестом; при несовпадении
   распаковка не производится (защита от повреждённой загрузки).
3. Если корпус-тексты (`txt/`) тоже обновлены — пересобрать локальный
   корпус по `CORPUS.md` и перепроверить цитаты (`make verify`).

## Альтернативная схема (не рекомендуется)

Серверная схема (RAG-API на отдельном хосте + публичный туннель/ngrok +
n8n) сохранена как опция в `server/` и `docs/REMOTE-RAG.md` — для
сред, где предпочтителен постоянный сетевой сервис, а не файловый обмен.